...
func void Info_TPL_1441_FirstWarn_Info()
{
	...
	AI_Output(self,hero,"Info_TPL_1441_FirstWarn_13_01");	//СТОЙ! Ты можешь вступить на земли храма только с разрешения одного из Гуру.
	...
	if(Npc_KnowsInfo(hero,PC_Psionic_SEND))
	{
		...
		Info_AddChoice(Info_TPL_1441_FirstWarn,"Мне дал разрешение сам Ю'Берион.",Info_TPL_1441_FirstWarn_Condition_YBERION);
		Info_AddChoice(Info_TPL_1441_FirstWarn,"Меня прислал послушник Лестер. ",Info_TPL_1441_FirstWarn_Condition_LESTER);
	}
	else
	{
...
{
	AI_Output(hero,self,"Info_TPL_1441_FirstWarn_Condition_YBERION_15_01");	//Мне дал разрешение сам Ю'Берион.
	AI_Output(self,hero,"Info_TPL_1441_FirstWarn_Condition_YBERION_13_02");	//Лжецам у нас обычно вырывают язык, так что хорошенько подумай, прежде чем говорить.
};

func void Info_TPL_1441_FirstWarn_Condition_LESTER()
{
	var C_Npc Templer;
	AI_Output(hero,self,"Info_TPL_1441_FirstWarn_Condition_LESTER_15_01");	//Меня прислал послушник Лестер. Я хочу предложить свои услуги Ю'Бериону.
	AI_Output(self,hero,"Info_TPL_1441_FirstWarn_Condition_LESTER_13_02");	//Я знаю Лестера. Мы ему доверяем. Ты можешь пройти.
	Info_ClearChoices(Info_TPL_1441_FirstWarn);
...
{
	AI_Output(self,hero,"Info_TPL_1441_LastWarn_13_01");	//Еще один шаг, и ты покойник!
	hero.aivar[AIV_LASTDISTTOWP] = Npc_GetDistToWP(hero,TPL_1441_CHECKPOINT);
