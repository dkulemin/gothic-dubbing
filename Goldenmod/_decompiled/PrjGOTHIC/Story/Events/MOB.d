...
var int abandonedmine_wheel_opened;

func int evt_abandonedmine_gate_trigger()
{
	PrintDebugNpc(PD_ITEM_MOBSI,"EVT_ABANDONEDMINE_GATE_TRIGGER");
	if((ABANDONEDMINE_WHEEL == TRUE) && (ABANDONEDMINE_WHEEL_OPENED == FALSE))
	{
		PrintDebugNpc(PD_ITEM_MOBSI,"...closed");
		AI_UseMob(hero,"VWHEEL",1);
		AI_UseMob(hero,"VWHEEL",-1);
		ABANDONEDMINE_WHEEL_OPENED = TRUE;
		return TRUE;
	}
	else
	{
		PrintDebugNpc(PD_ITEM_MOBSI,"...open");
		if(!ABANDONEDMINE_WHEEL_OPENED)
		{
			G_PrintScreen(_STR_MESSAGE_WHEEL_STUCKS);
		};
	};
