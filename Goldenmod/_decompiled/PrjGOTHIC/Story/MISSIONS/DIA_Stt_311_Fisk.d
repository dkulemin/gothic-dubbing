...
func int Stt_311_Fisk_MordragKO_Condition()
{
	...
	if(((MordragKO_HauAb == TRUE) || (Npc_KnowsInfo(hero,Org_826_Mordrag_Fight) && Npc_IsDead(Mordrag))) && (Npc_GetTrueGuild(hero) == GIL_None))
	{
		return 1;
	};
};
...
func void Stt_311_Fisk_MordragKO_Info()
{
	...
	if(Npc_KnowsInfo(hero,Org_826_Mordrag_Fight) && Npc_IsDead(Mordrag))
	{
		AI_Output(self,other,"Stt_311_Fisk_MordragKO_12_03");	//Я слышал, что ты поссорился с Мордарагом. И я подозреваю, что ты убил его!
	};
...
instance STT_311_FISK_QUENTIN(C_Info)
{
	npc = Stt_311_Fisk;
	nr = 998;
	condition = stt_311_fisk_quentin_condition;
	information = stt_311_fisk_quentin_info;
	permanent = 0;
	description = "Что ты знаешь о доставке товаров в шахту?";
};


func int stt_311_fisk_quentin_condition()
{
	if(GQ_OK == TRUE)
	{
		return 1;
...
func void stt_311_fisk_quentin_info()
{
	AI_Output(other,self,"Stt_311_Fisk_Quentin_15_00");	//Что ты знаешь о доставке товаров в шахту?
	AI_Output(self,other,"Stt_311_Fisk_Quentin_12_01");	//То, что закажет Ян.
	AI_Output(self,other,"Stt_311_Fisk_Quentin_12_02");	//Он составит список товаров для стражников и рудокопов.
	B_LogEntry(CH1_GQW,"Список товаров находится у Яна.");
};
