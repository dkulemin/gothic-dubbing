...
func void DIA_Gomez_Hello_Kontakte()
{
	...
	Info_AddChoice(DIA_Gomez_Hello,"С Ларесом.",DIA_Gomez_Hello_Kontakte_Lares);
...
{
	AI_Output(other,self,"DIA_Gomez_Hello_Kontakte_Lares_15_00");	//Ларес.
...
func void DIA_Gomez_Dabei_Info()
{
	...
	AI_Output(self,other,"DIA_Gomez_Dabei_11_02");	//Ворон введет тебя в курс дела.
	...
	B_LogEntry(CH1_JoinOC,"С сегодняшнего дня я работаю на Гомеза и Старый Лагерь. Детали мне расскажет Ворон.");
	Log_SetTopicStatus(CH1_JoinOC,LOG_SUCCESS);
	Log_CreateTopic(CH1_JoinNC,LOG_MISSION);
	Log_SetTopicStatus(CH1_JoinNC,LOG_FAILED);
	B_LogEntry(CH1_JoinNC,"Поскольку я принадлежу к людям Гомеза, банда Лареса теперь не для меня!");
	...
	if(HORATIO_LORD == TRUE)
	{
		...
		B_LogEntry(CH1_HORATIO_OK,"Теперь, когда я присоединился к Старому лагерю, мое обещание побить лорда и его головорезов ничего не стоит.");
		HORATIO_LORD = FALSE;
		NA_GIL_SLD = FALSE;
	};
	AI_StopProcessInfos(self);
...
func void DIA_Gomez_NurSo_Info()
{
	...
	AI_Output(self,other,"DIA_Gomez_NurSo_11_00");	//Доложи Ворону. Никогда не обращайся ко мне без разрешения!
};
