...
func void B_SetHeroAttributes(var int selection,var int increase,var int LP_Cost)
{
	...
	if(hero.lp >= LP_Cost)
	{
		...
		Attributes = ConcatStrings(Attributes,"Мана:");
		Attributes = ConcatStrings(Attributes,IntToString(hero.attribute[ATR_MANA_MAX]));
		PrintScreen(Attributes,-1,-1,"font_old_20_white.tga",3);
	}
	else
	{
		PrintScreen("Не хватает очков обучения!",-1,-1,"FONT_OLD_20_WHITE.TGA",2);
...
instance CH_Guild(C_Info)
{
	...
	important = 0;
	permanent = 1;
	description = "Установить гильдию!";
};
...
func void CH_Guild_Info()
{
	...
	Info_AddChoice(CH_Guild,"Гуру",ch_guru);
...
func void ch_guru()
{
	var C_Npc novescapea;
	var C_Npc novescapeb;
	Info_ClearChoices(CH_Guild);
	hero.guild = GIL_GUR;
	...
	Mdl_RemoveOverlayMds(hero,"Humans_Militia.mds");
	Mdl_RemoveOverlayMds(hero,"Humans_Relaxed.mds");
	novescapea = Hlp_GetNpc(nov_9000_escaped);
	novescapeb = Hlp_GetNpc(nov_9001_escaped);
	Wld_InsertNpc(novescapea,"NOV_1");
	Wld_InsertNpc(novescapeb,"NOV_2");
};
...
func void CH_Training_STR_Info()
{
	...
	Info_AddChoice(CH_Training_STR,"Сила (максимальная)",CH_Strength_all);
	Info_AddChoice(CH_Training_STR,"Сила (подогнать под оружие)",CH_Strength_Weapon);
...
func void CH_Strength_Weapon()
{
	...
	{
		PrintScreen("nцtige STR bereits vorhanden",-1,-1,"font_old_20_white.tga",10);
	};
};
...
func void CH_Training_DEX_Info()
{
	...
	Info_AddChoice(CH_Training_DEX,"Ловкость (максимальная)",CH_Dexterity_all);
	Info_AddChoice(CH_Training_DEX,"Ловкость (подогнать под оружие)",CH_Dexterity_Weapon);
	if(hero.lp >= 20)
	{
...
func void CH_Dexterity_Weapon()
{
	...
	{
		PrintScreen("nцtige DEX bereits vorhanden",-1,-1,"font_old_20_white.tga",10);
	};
};
...
func void CH_Training_Mana_Info()
{
	...
	Info_AddChoice(CH_Training_Mana,"Мана (максимальная)",CH_Mana_all);
	if(hero.lp >= 20)
	{
