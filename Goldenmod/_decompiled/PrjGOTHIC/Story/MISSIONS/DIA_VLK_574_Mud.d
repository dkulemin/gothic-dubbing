...
func void DIA_Mud_FirstEXIT_Info()
{
	...
	{
		AI_Output(other,self,"DIA_Mud_FirstEXIT_15_00");	//Я тут кое-что узнал о твоем доме.
		AI_Output(self,other,"DIA_Mud_FirstEXIT_07_02");	//Да? И что ты узнал?
	}
	else
	{
...
instance DIA_MUD_HOUSE(C_Info)
{
	...
	permanent = 0;
	description = "(уговорить поменяться домами)";
};
...
{
	AI_Output(other,self,"DIA_Mud_House_15_00");	//Сегодня я прочитал древние письмена, в которых говорится о страшном проклятьи, которое наложено на этот дом...
	AI_Output(self,other,"DIA_Mud_House_07_00");	//(испуганно) Правда?
	AI_Output(other,self,"DIA_Mud_House_15_01");	//Честное слово!
	AI_Output(self,other,"DIA_Mud_House_07_01");	//(бледнея) И что же мне теперь делать?!
	AI_Output(other,self,"DIA_Mud_House_15_02");	//Тебе повезло, я могу помочь тебе поменятся с одним рудокопом домами.
	AI_Output(self,other,"DIA_Mud_House_07_02");	//Отлично!
	AI_Output(other,self,"DIA_Mud_House_15_03");	//Дом находится у котлована справа от ворот, в нем 2 входа.
	AI_Output(self,other,"DIA_Mud_House_07_03");	//Хорошо. Сегодня же я уйду отсюда.
	B_LogEntry(CH1_KYLE_PROBLEM,"Этот Мад сразу же согласился переехать в дом Кайли.");
	B_GiveXP(200);
...
instance DIA_MUD_NEW_HOUSE(C_Info)
{
	...
	permanent = 0;
	description = "Нравится ли тебе новый дом?";
};
...
{
	AI_Output(other,self,"DIA_Mud_New_House_15_00");	//Нравится ли тебе новый дом?
	AI_Output(self,other,"DIA_Mud_New_House_07_00");	//Прекрасный дом. Только настоящий глупец мог обменять мой проклятый дом на этот.
};
...
{
	AI_Output(self,other,"DIA_Mud_New_House_Follow_07_00");	//Я прогуляюсь с тобой по лагерю, хорошо?
	KYLE_TO_MUD = FALSE;
