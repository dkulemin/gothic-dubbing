...
func void ORG_801_Lares_BringList_Info()
{
	...
	B_LogEntry(CH1_JoinNC,"Ларес, лидер воров, был очень удивлен, когда увидел список. Он дополнил его, и теперь я должен отнести его Диего.");
...
func void ORG_801_Lares_BringListBack_Info()
{
	...
	B_LogEntry(CH1_JoinNC,"Ларес уже ограбил конвой из Старого Лагеря. Совпадение? Или расчет? Вот в чем вопрос.");
...
func void ORG_801_Lares_MordragSentMe_Info()
{
	...
	B_LogEntry(CH1_JoinNC,"Ларес послал меня к Мордрагу. Он хочет, чтобы я передал ему следующее: в Лагерь Братства будет заслан шпион с целью выяснения планов Братства.");
...
func void ORG_801_Lares_GetKraut_Info()
{
	...
	B_LogEntry(CH1_JoinNC,"Мне было сказано взять болотник Идола Исидро и продать его за 400 кусков руды. Ларес хочет получить прибыль.");
};
...
func void ORG_801_Lares_400Ore_Info()
{
	...
	if(Npc_HasItems(other,ItMiNugget) >= 400)
	{
		...
		B_LogEntry(CH1_JoinNC,"Ларес не смог скрыть своего удовлетворения, когда я дал ему 400 кусков руды, вырученные с продажи болотника Исидро.");
...
func void ORG_801_Lares_Reicht_Info()
{
	...
	if((Points_NC >= 30) && (hero.level >= 5))
	{
		...
		B_LogEntry(CH1_JoinNC,"Ларес принял меня в банду. Теперь Новый Лагерь - это мой дом!");
		Log_SetTopicStatus(CH1_JoinNC,LOG_SUCCESS);
		Log_CreateTopic(CH1_JoinOC,LOG_MISSION);
		Log_SetTopicStatus(CH1_JoinOC,LOG_FAILED);
		B_LogEntry(CH1_JoinOC,"Участник банды Лареса не может присоединиться к Старому Лагерю. Это просто невозможно.");
		...
		if(HORATIO_LORD == TRUE)
		{
			...
			B_LogEntry(CH1_HORATIO_OK,"Теперь, когда я стал одним из головорезов, мое обещание побить лорда ничего не стоит.");
			HORATIO_LORD = FALSE;
			NA_GIL_SLD = FALSE;
		};
...
func void ORG_801_Lares_WhereLearn_Info()
{
	...
	B_LogEntry(GE_TeacherNC,"Ларес может помочь мне потренировать и силу, и умение.");
};
