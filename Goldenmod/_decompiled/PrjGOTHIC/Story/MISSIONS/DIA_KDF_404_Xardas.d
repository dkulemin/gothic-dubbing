...
{
	if(Npc_KnowsInfo(hero,Info_Xardas_FORMULA) && ((Npc_GetTrueGuild(hero) == GIL_KDF) || (Npc_GetTrueGuild(hero) == GIL_KDW) || (Npc_GetTrueGuild(hero) == GIL_DMB) || (Npc_GetTrueGuild(hero) == GIL_GUR)))
	{
		return TRUE;
	};
};
...
{
	if(EnteredTemple && ((Npc_GetTrueGuild(hero) == GIL_KDF) || (Npc_GetTrueGuild(hero) == GIL_KDW) || (Npc_GetTrueGuild(hero) == GIL_GUR)))
	{
		return TRUE;
	};
};
...
func void Info_Xardas_LOADSWORD09_Info()
{
	...
	{
		if((Npc_GetTrueGuild(hero) == GIL_KDF) || (Npc_GetTrueGuild(hero) == GIL_KDW))
		{
		...
		{
			AI_Output(self,other,"Info_Xardas_LOADSWORD09_14_08");	//Не сейчас. У тебя есть все навыки, но ты еще недостаточно опытен. Пусть сначала с тобой позанимается Ю'Берион.
		};
		AI_StopProcessInfos(self);
	};
};
