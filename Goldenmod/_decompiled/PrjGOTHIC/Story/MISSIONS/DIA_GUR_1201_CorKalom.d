...
func void GUR_1201_CorKalom_Experimente_Info()
{
	...
	AI_Output(self,other,"GUR_1201_CorKalom_AboutExperimente_10_00");	//Я готовлю эликсиры для предстоящей церемонии вызова Спящего.
};
	...
	if(hero.level >= 5)
	{
		if(counter >= 4)
		{
			...
			if(HORATIO_LORD == TRUE)
			{
				...
				B_LogEntry(CH1_HORATIO_OK,"Теперь, когда я присоединился к Болотному лагерю, мое обещание побить лорда и его головорезов ничего не стоит.");
				HORATIO_LORD = FALSE;
				NA_GIL_SLD = FALSE;
			};
			GQ = TRUE;
...
func void Info_CorKalom_BringBook_Success_Info()
{
	...
	AI_Output(self,other,"Info_CorKalom_BringBook_Success_10_06");	//Возьми этот эликсир. Он поможет тебе подготовиться к церемонии.
...
instance GUR_1201_CORKALOM_ALCHEMY(C_Info)
{
	npc = GUR_1201_CorKalom;
	nr = 2;
	condition = gur_1201_corkalom_alchemy_condition;
	information = gur_1201_corkalom_alchemy_info;
	permanent = 1;
	description = "Ты можешь научить меня готовить зелья?";
};


func int gur_1201_corkalom_alchemy_condition()
{
	if(Npc_KnowsInfo(hero,GUR_1201_CorKalom_Experimente) && (ALCHEMY_BY_CORKALOM == FALSE) && (Kapitel <= 2))
	{
		return 1;
	};
};

func void gur_1201_corkalom_alchemy_info()
{
	AI_Output(other,self,"GUR_1201_CorKalom_ALCHEMY_15_00");	//Ты можешь научить меня готовить зелья?
	if((Npc_GetTrueGuild(hero) == GIL_NOV) || (Npc_GetTrueGuild(hero) == GIL_TPL))
	{
		AI_Output(self,other,"GUR_1201_CorKalom_ALCHEMY_10_00");	//Поскольку ты являешься членом братства, я буду учить тебя.
		if(Kapitel == 2)
		{
			AI_Output(self,other,"GUR_1201_CorKalom_ALCHEMY_10_01");	//Я вижу, ты готов начать обучение.
			Log_CreateTopic(GE_TeacherPSI,LOG_NOTE);
			B_LogEntry(GE_TeacherPSI,"Кор Галом обучает членов братства искусству алхимии.");
			ALCHEMY_BY_CORKALOM = TRUE;
			gur_1201_corkalom_alchemy.permanent = 0;
		}
		else
		{
			AI_Output(self,other,"GUR_1201_CorKalom_ALCHEMY_10_02");	//Но ты еще недостаточно опытен.
		};
	}
	else
	{
		AI_Output(self,other,"GUR_1201_CorKalom_ALCHEMY_10_03");	//Я обучаю только членов священного братства Спящего.
	};
};


instance GUR_1201_CORKALOM_LEARN_ALCHEMY(C_Info)
{
	npc = GUR_1201_CorKalom;
	nr = 2;
	condition = gur_1201_corkalom_learn_alchemy_condition;
	information = gur_1201_corkalom_learn_alchemy_info;
	permanent = 1;
	description = "Я хочу изучить алхимию. (10 очков обучения)";
};


func int gur_1201_corkalom_learn_alchemy_condition()
{
	if((ALCHEMY_BY_CORKALOM == TRUE) && (HERO_KNOWS_ALCHEMY == FALSE) && (Kapitel <= 2))
	{
		return 1;
	};
};

func void gur_1201_corkalom_learn_alchemy_info()
{
	AI_Output(other,self,"GUR_1201_CorKalom_Learn_ALCHEMY_15_00");	//Я хочу изучить алхимию.
	if(hero.lp >= 10)
	{
		hero.lp = hero.lp - 10;
		AI_Output(self,other,"GUR_1201_CorKalom_Learn_ALCHEMY_10_00");	//Я не могу доверить тебе приговление эликсиров для предстоящей церемонии...
		AI_Output(self,other,"GUR_1201_CorKalom_Learn_ALCHEMY_10_01");	//... но я покажу тебе несколько несложных рецептов.
		AI_Output(self,other,"GUR_1201_CorKalom_Learn_ALCHEMY_10_02");	//Тебе понадобится лабораторный флакон и ингредиенты для зелья.
		AI_Output(self,other,"GUR_1201_CorKalom_Learn_ALCHEMY_10_03");	//Будь осторожен при кипячении некоторых эликсиров.
		HERO_KNOWS_ALCHEMY = TRUE;
		Log_CreateTopic(GE_POTIONALCHEMY,LOG_NOTE);
		B_LogEntry(GE_POTIONALCHEMY,"Гуру Кор Галом научил меня приговлению зелий.");
		gur_1201_corkalom_learn_alchemy.permanent = 0;
	}
	else
	{
		AI_Output(self,other,"SVM_10_NoLearnNoPoints");	//Я не могу учить тебя, ты слишком неопытен.
	};
};


instance GUR_1201_CORKALOM_LEARN_POTIONS(C_Info)
{
	npc = GUR_1201_CorKalom;
	nr = 1;
	condition = gur_1201_corkalom_learn_potions_condition;
	information = gur_1201_corkalom_learn_potions_info;
	important = 0;
	permanent = 1;
	description = "(алхимия)";
};
...
func int gur_1201_corkalom_learn_potions_condition()
{
	if((HERO_KNOWS_ALCHEMY == TRUE) && (ALCHEMY_BY_CORKALOM == TRUE))
	{
		return TRUE;
	};
};

func void gur_1201_corkalom_learn_potions_info()
{
	Info_AddChoice(gur_1201_corkalom_learn_potions,"НАЗАД",gur_1201_corkalom_learn_potions_back);
	if(HERO_KNOWS_HASTE_1 == FALSE)
	{
		Info_AddChoice(gur_1201_corkalom_learn_potions,"Эликсир проворства. (1 очко обучения)",gur_1201_corkalom_learn_potions_haste);
	};
	if(HERO_KNOWS_MANA_1 == FALSE)
	{
		Info_AddChoice(gur_1201_corkalom_learn_potions,"Эссенция магической энергии. (1 очко обучения)",gur_1201_corkalom_learn_potions_mana);
	};
	if(HERO_KNOWS_HEALTH_1 == FALSE)
	{
		Info_AddChoice(gur_1201_corkalom_learn_potions,"Эссенция исцеления. (1 очко обучения)",gur_1201_corkalom_learn_potions_health);
	};
};

func void gur_1201_corkalom_learn_potions_health()
{
	AI_Output(other,self,"GUR_1201_CorKalom_Learn_POTIONS_HEALTH_15_00");	//... эссенция исцеления.
	if(hero.lp >= 1)
	{
		hero.lp = hero.lp - 1;
		AI_Output(self,other,"GUR_1201_CorKalom_Learn_POTIONS_HEALTH_14_00");	//Тебе нужно смешать 3 куста целительныой травы.
		B_LogEntry(GE_POTIONALCHEMY,"Для приготовления эссенции исцеления мне понадобятся 3 куста целительныой травы.");
		HERO_KNOWS_HEALTH_1 = TRUE;
	}
	else
	{
		AI_Output(self,other,"SVM_10_NoLearnNoPoints");	//Я не могу учить тебя, ты слишком неопытен.
	};
	Info_ClearChoices(gur_1201_corkalom_learn_potions);
};

func void gur_1201_corkalom_learn_potions_mana()
{
	AI_Output(other,self,"GUR_1201_CorKalom_Learn_POTIONS_MANA_15_00");	//... эссенция магической энергии.
	if(hero.lp >= 1)
	{
		hero.lp = hero.lp - 1;
		AI_Output(self,other,"GUR_1201_CorKalom_Learn_POTIONS_MANA_14_00");	//Нужно просто смешать 3 куста темной травы.
		B_LogEntry(GE_POTIONALCHEMY,"Для приготовления эссенции магической энергии мне понадобятся 3 куста темной травы.");
		HERO_KNOWS_MANA_1 = TRUE;
	}
	else
	{
		AI_Output(self,other,"SVM_10_NoLearnNoPoints");	//Я не могу учить тебя, ты слишком неопытен.
...
func void gur_1201_corkalom_learn_potions_haste()
{
	AI_Output(other,self,"GUR_1201_CorKalom_Learn_POTIONS_HASTE_15_00");	//... эликсир проворства.
	if(hero.lp >= 1)
	{
		hero.lp = hero.lp - 1;
		AI_Output(self,other,"GUR_1201_CorKalom_Learn_POTIONS_HASTE_14_00");	//Ты должен смешать серафис с солнечником.
		B_LogEntry(GE_POTIONALCHEMY,"Для приготовления эликсира проворства мне понадобятся серафис и солнечник.");
		HERO_KNOWS_HASTE_1 = TRUE;
	}
	else
	{
		AI_Output(self,other,"SVM_10_NoLearnNoPoints");	//Я не могу учить тебя, ты слишком неопытен.
...
func void gur_1201_corkalom_learn_potions_back()
{
	AI_Output(other,self,"GUR_1201_CorKalom_Learn_POTIONS_BACK_15_00");	//Я зайду в другой раз.
	Info_ClearChoices(gur_1201_corkalom_learn_potions);
};
