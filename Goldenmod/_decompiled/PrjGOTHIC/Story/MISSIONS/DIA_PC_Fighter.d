...
func void Dia_Gorn_Hut_Info()
{
	...
	AI_Output(self,other,"DIA_Gorn_Hut_09_04");	//Ну, нам, наемникам. Наемники и воры живут раздельно, и ни один из шайки Лареса не должен появляться по эту сторону от большой дыры.
...
instance DIA_GORN_BDT(C_Info)
{
	...
	permanent = 0;
	description = "Ты не знаешь, где тут можно подзаработать?";
};
...
{
	AI_Output(other,self,"DIA_Gorn_BDT_15_00");	//Ты не знаешь, где тут можно подзаработать?
	AI_Output(self,other,"DIA_Gorn_BDT_09_00");	//Да, ты можешь поработать на Квентина.
	if(Npc_KnowsInfo(hero,org_858_quentin_guard))
	{
		AI_Output(other,self,"DIA_Gorn_BDT_15_01");	//Я знаю Квентина.
		AI_Output(self,other,"DIA_Gorn_BDT_09_01");	//Тогда ты сам сможешь его найти.
		QUENTIN_WORK = TRUE;
	}
	else
	{
		AI_Output(other,self,"DIA_Gorn_BDT_15_02");	//А кто такой, этот Квентин?
		if(GQ == TRUE)
		{
			AI_Output(self,other,"DIA_Gorn_BDT_09_02");	//Он главарь шайки бандитов, грабящих людей из Старого Лагеря.
		}
		else
		{
			AI_Output(self,other,"DIA_Gorn_BDT_09_03");	//Я тебе не скажу, вдруг ты шпион Cтарого лагеря.
		};
	};
};
...
instance DIA_GORN_BDT_WERE(C_Info)
{
	...
	permanent = 0;
	description = "Так где я могу найти Квентина?";
};
...
{
	AI_Output(other,self,"DIA_Gorn_BDT_WERE_15_00");	//Так где я могу найти Квентина?
	AI_Output(self,other,"DIA_Gorn_BDT_WERE_09_00");	//Сейчас Квентин осматривает местность вокруг шахты, ты можешь найти его там.
	QUENTIN_WORK = TRUE;
	AI_StopProcessInfos(self);
...
func void Info_Gorn_RUINJOIN_Info()
{
	...
	B_LogEntry(CH3_MonasteryRuin,"Мы продолжим поиски вместе. Горн предупредил о больших стаях глорхов, бродящих по округе.");
...
func void Info_Gorn_FMWATCH_Info()
{
	...
	{
		AI_Output(self,hero,"Info_Gorn_FMWATCH_09_05");	//Ли прислал покрепление. Теперь все под контролем.
		if(GORNHELPOK != TRUE)
		{
			GORNHELPOK = TRUE;
			B_GiveXP(1000);
			Log_SetTopicStatus(CH4_GORNHELP,LOG_SUCCESS);
			B_LogEntry(CH4_GORNHELP,"Горн получил подкрепление. Теперь он может справиться со стражниками и без моей помощи.");
		};
	}
	else
	{
		...
		if(GORNHELP != TRUE)
		{
			...
			B_LogEntry(CH4_GORNHELP,"Горн хочет, чтобы Ли прислал ему подкрепление.");
		};
	};
};
...
{
	AI_Output(self,hero,"Info_Gorn_PROHOD_WATCH_09_02");	//Постой. Мы должны найти проход в горах, чтобы не допустить повторную атаку стражников.
	AI_Output(hero,self,"Info_Gorn_PROHOD_WATCH_15_01");	//Что, опять я?
	AI_Output(self,hero,"Info_Gorn_PROHOD_WATCH_09_03");	//А на кого я еще могу положиться?
	AI_Output(self,hero,"Info_Gorn_PROHOD_WATCH_09_04");	//Иди вперед.
	Log_CreateTopic(CH4_GORNPROHOD,LOG_MISSION);
	Log_SetTopicStatus(CH4_GORNPROHOD,LOG_RUNNING);
	B_LogEntry(CH4_GORNPROHOD,"Горн хочет найти проход в горах, чтобы предотвратить повторную атаку стражников.");
...
{
	AI_GotoNpc(self,hero);
	AI_Output(self,hero,"Info_Gorn_PROHOD_FOUND_09_02");	//Похоже, что стражники пришли отсюда.
	AI_Output(hero,self,"Info_Gorn_PROHOD_FOUND_15_01");	//Да, они могли пройти через земли орков.
	AI_Output(self,hero,"Info_Gorn_PROHOD_FOUND_09_03");	//Ты должен сообщить об этом Ли.
	AI_Output(self,hero,"Info_Gorn_PROHOD_FOUND_09_04");	//А я еще немного побуду здесь.
	AI_Output(hero,self,"Info_Gorn_PROHOD_FOUND_15_02");	//Ладно, я пошел.
	B_GiveXP(500);
	B_LogEntry(CH4_GORNPROHOD,"Мы с Горном нашли проход, по которому пришли стражники.");
