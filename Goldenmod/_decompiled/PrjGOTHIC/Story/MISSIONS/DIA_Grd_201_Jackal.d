...
instance INFO_JACKAL_GILBERT(C_Info)
{
	...
	permanent = 0;
	description = "Ты помнишь типа по имени Жильбер?";
};
...
{
	AI_Output(other,self,"Info_Jackal_Gilbert_15_00");	//Ты помнишь типа по имени Жильбер?
	if(Npc_KnowsInfo(hero,dia_gravo_gilbert_about))
	{
		AI_Output(other,self,"Info_Jackal_Gilbert_15_01");	//Он был рудокопом и работал в кузнице замка.
	};
	AI_Output(self,other,"Info_Jackal_Gilbert_07_00");	//Ах, да! Я его помню. Мерзкий тип. Все время ссорился с парнями и нарывался на драки.
	AI_Output(other,self,"Info_Jackal_Gilbert_15_02");	//Тогда ты должен знать, что он украл у Торуса.
	AI_Output(self,other,"Info_Jackal_Gilbert_07_01");	//Эй, парень, потише. Здесь не принято говорить о таких вещах вслух. К тому же, зачем тебе это нужно знать?
	AI_Output(other,self,"Info_Jackal_Gilbert_15_03");	//Торус поручил мне вернуть украденное.
	AI_Output(self,other,"Info_Jackal_Gilbert_07_02");	//Да? Ты должен гордится этим. Торус мало кому доверяет.
	AI_Output(other,self,"Info_Jackal_Gilbert_15_04");	//Так ты поможешь мне?
	AI_Output(self,other,"Info_Jackal_Gilbert_07_03");	//Хорошо. Что ты хочешь узнать?
	AI_Output(other,self,"Info_Jackal_Gilbert_15_05");	//ЧТО ИМЕННО украл Жильбер у Торуса?
	AI_Output(self,other,"Info_Jackal_Gilbert_07_04");	//А Торус не сказал тебе об этом?
	AI_Output(other,self,"DIA_Orry_GuardGate_No_15_00");	//Нет.
	AI_Output(self,other,"Info_Jackal_Gilbert_07_05");	//Тогда извини, парень, я тоже промолчу.
	B_LogEntry(CH1_GILBERT,"Шакал что-то знает, но он не хочет говорить мне об этом.");
};
